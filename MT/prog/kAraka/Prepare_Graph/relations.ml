value relations_encodings = [
	("niwya_sambanXaH",101);
	("niwya_sambanXaH1",102);
	("prawiyogI",300);
	("BAvalakRaNasapwamI_samAnakAlaH",3);
	("BAvalakRaNasapwamI_pUrvakAlaH",4);
	("BAvalakRaNasapwamI_ananwarakAlaH",5);
	("karwA_be_verbs",6);
	("karwA",7);
	("karwqrahiwakarwqsamAnAXikaraNam",8);
	("viXeya_viSeRaNam",9);
	("muKyakarma",10);
	("gONakarma",11);
	("vAkyakarma",12);
	("karmasamAnAXikaraNam",13);
	("karma",14);
	("prayojakakarwA",15);
	("prayojyakarwA",16);
	("maXyasWakarwA",17);
	("karaNam",18);
	("sampraxAnam",19);
	("apAxAnam",20);
	("viRayAXikaraNam",21);
	("xeSAXikaraNam",22);
	("kAlAXikaraNam",23);
	("aXikaraNam",24);
	("pUrvakAlaH",25);
(* earlier it was pUrvakAla = 24, and aXikaraNa = 25. But now we want to impose a condition that there can be only one adhikarana, hence the order changed *)
	("kriyAviSeRaNam",26);
	("nirXAraNam",27);
	("sambanXaH",28);
	("ananwarakAlaH",30);
	("samboXana_xyowakaH",31);
	("samucciwaH",32);
	("sup_samucciwaH",33);
	("anyawaraH",34);
	("sup_anyawaraH",35);
	("viSeRaNam",36);
	("aBexaH",37);
	("RaRTIsambanXaH",38);
	("hewuH",39);
	("hewuH5",29);
	("prayojanam1",40);
	("prayojanam",41);
	("varwamAnasamAnakAlaH",42);
	("sanxarBa_binxuH",43);
	("wulanA_binxuH",44);
	("samuccaya_xyowakaH",45);
	("sup_samuccaya_xyowakaH",46);
	("anyawara_xyowakaH",47);
	("sup_anyawara_xyowakaH",48);
	("samboXyaH",49);
	("prawiReXaH",50);
	("wIvrawAxarSI",51);
	("saFjFA_xyowakaH",52);
	("saFjFA",53);
	("Gataka",54 );
	("Gataka_xyowakaH",55);
	("kAraNa_xyowakaH",56);
	("kArya_xyowakaH",57);
	("vyaBicAraH",58);
	("vyaBicAra_xyowakaH",59);
	("kArya_kAraNa_BAvaH",60);
	("prayojakakarwqrahiwaprayojyakarwA",1);
	("viroXakaH",68);
	("viroXa_xyowakaH",69);
	("samAsaH",71);
	("samAnakAlaH",74);
	("prayojanam_xyowakaH",75);
	("sahArWa_xyowakaH",76);
	("vinArWa_xyowakaH",77);
	("lyapkarmAXikaraNam",78);
	("upamAnam",79);
	("upamAna_xyowakaH",80);
	("wumunkarma",81);
	("vIpsA",82);
	("apAxAnam_vIpsA",83);
	("karaNam_vIpsA",84);
	("uxgAravAcakaH",85);
	("svAmI",86);
	("aXyAhqwakriyAkarma",87);
	("afgavikAraH",88);
        ("uwprekRA_xyowakaH",89);
	("ZavaXiH",91);
	("sahArWaH",92);
	("vinArWaH",93);
	("AramBa_binxuH",94);
	("ABimuKyam",95);
	("apavarga_sambanXaH",96);
	("vAkyakarma_xyowakaH",97);
	("ZiwWamBUwaH",98);
	("awyanwasaMyoga_sambanXaH",99);
	("upa_apekRA",2000);
	("upa_karma",2200);
	("upa_karmapravacanIyaH",2300);
	("upa_ABimuKyam",2400);
	("upa_nirXAraNam",2500);
	("upa_prawisixXaH",2600);
	("upa_AramBa_binxuH",2900);
	("upa_svAmI",3000);
	("upa_vinA",3200);
	("upa_viRayAXikaraNam",3300);
	("apAxAnam_upa",4000);
	("xeSAXikaraNam_upa",4100);
	("karwqsamAnAXikaraNam_upa",4200);
	("sambanXa_upa",4300);
	("karwA_upa",4400);
	("aXikaraNam_upa",4500);
	("gawikarwA",200);
	("gawikarma",201);
	("BkarwA",202);
	("Bkarma",203);
	("iRkarma",214);
	("AvaSyakawA_xyowakaH",206);
	("pariNAma_xyowakaH",207);
	("AvaSyakawA_pariNAma_sambanXaH",209);
	("samAnAXikaraNam",1009);
	("upamAnam_rahiwa_upamAna_xyowakaH",1080);
	("A1",1010);
	("A2",1011);
	("A3",1012);
	("A7",1016);
	("wa_pu_2",1022);
	("wa_pu_3",1023);
	("wa_pu_4",1024);
	("wa_pu_5",1025);
	("wa_pu_7",1027);
	("wa_pu", 1028);
	("ka_XA_1", 1029);
	("ka_XA", 1030);
	("ka_XA_2", 1031);
	("ka_XA_3", 1032);
	("ka_XA_4", 1033);
	("ka_XA_5", 1034);
	("naF_wa_pu", 1035);
	("wa_pu_xvigu", 1036);
	("u_pa_wa_pu", 1037);
	("ba_vrI", 1038);
	("wa_pu_6", 1039);
	("xvanxva", 1040);
        ]
;

